# Pipeline for creation of meta.xml files for Darwin Core archives
...
assign_uri <- function(term) {
  ...
  if(is.null(try_dwc$error) & !is.null(try_dwc$result)) {
    ...
    if(is.null(try_dc$error) & !is.null(try_dc$result)) {
      ...
      # Note: within Dublin Core some terms are not part of the "/elements/1.1/" namespace but of the "/terms/" namespace
      url <- paste0("https://www.dublincore.org/specifications/dublin-core/dcmi-terms/terms/", term)
      try_dc <- try_url(url)
